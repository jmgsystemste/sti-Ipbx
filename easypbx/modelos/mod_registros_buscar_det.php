...
class mod_resgistros_buscar_det {
    ...
    public static function BuscarRegistros() {
        ...
        $consulta = "SELECT id, calldate, src, dst, duration, disposition, userfield FROM cdr WHERE calldate>='" . self::$thedate1 . "' AND calldate<='" . self::$thedate2 . "' AND disposition='" . self::$estado . "' ORDER BY id LIMIT " . self::$ini_reg . "," . self::$can_reg;
        $rows = self::consulta_Bd($consulta);
        $consulta01 = "SELECT COUNT(*) FROM cdr WHERE calldate>='" . self::$thedate1 . "' AND calldate<='" . self::$thedate2 . "' AND disposition='" . self::$estado . "'";
...
mod_resgistros_buscar_det::BuscarRegistros();
